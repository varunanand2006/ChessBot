# Numbers & interesting finds

A collection of the flashy numbers this project produced, plus some genuinely
interesting endgame truths that fell out of the GPU-generated 5-piece tablebases.
Every tablebase figure below was cross-checked against the Lichess (Gaviota DTM)
tablebase — see [`cuda/PROFILING.md`](cuda/PROFILING.md) for the full record.
...
## Interesting finds (from the 28 five-piece tablebases)

The tablebases store *perfect play* — exact distance-to-mate for every legal
position. Ranking the 28 materials by how often the game is a **draw** (a metric
that doesn't depend on whose move it is) surfaces some real chess truths.
...
These aren't guesses — they're the exact fraction of drawn positions over
*every* legal configuration (156–175 million each). R+minor vs R and two-minors
vs one being "usually drawn" is textbook endgame theory, here confirmed
empirically to the position.

### 2. The single deepest mate hides inside a *mostly-drawn* ending

**KBN vs KN is 82.9% draws — yet its won positions include a forced mate-in-107
(213 plies)**, the deepest in the entire set and one of the deepest pawnless
5-man mates known. So the hardest, most precise win in all 28 tables lives in a
material that is *usually* a draw: you almost never get the win, and when you do,
it demands 107 moves of flawless play. (Matched to Gaviota exactly.)

### 3. Overwhelming material is *not* a fast mate — the queen is what forces it

Three of the four "vs a lone king" materials mate almost instantly, but one is
wildly different:
...
pieces, yet its worst-case mate is **~6× longer** — because no single piece
forces the mate, and the extra pieces make stalemate a constant hazard. More
material can mean a *longer* forced mate.
...
Being up a bishop or knight in a queen ending wins only about **half** the time —
the defending queen generates enough perpetual-check and fortress resources to
hold the other half.

### 5. Rook + minor vs queen: the queen usually wins, and the fights are long
...
wins are among the deepest in the set (mate-in-69/70) — long technical grinds,
not quick kills.
