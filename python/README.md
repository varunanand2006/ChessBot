# The Python engine — the origin

This directory holds the original chess engine, written from scratch in Python
(~1600 ELO): a terminal interface, a Pygame GUI, and a Texel-tuning pipeline for
the evaluation. It is the starting point the C++/CUDA project in the repo root
grew out of, preserved unchanged — the C++ engine mirrors its architecture, made
bitboard-idiomatic and much faster.

The sections below document each feature in the order it was added, with the
purpose and the speed/strength tradeoff of each — the chain of ideas that turns a
naive exhaustive search into a ~1600 ELO engine.

> Paths below are relative to this `python/` directory.
...
The GUI (`gui.py`) runs under CPython for Pygame compatibility. It launches
`engine_main.py` as a PyPy subprocess and communicates over stdin/stdout, so the
engine still runs at full PyPy speed.
...
| `gen_eval_tables.py` | Emits the C++ engine's eval tables from these constants |
| `verify_tablebase.py` | Checks generated tablebases against the Lichess API |
| `texel_*.py` | The Texel-tuning pipeline (data generation + training) |

## How it works

Each feature below was added incrementally. Understanding the chain — and the
tradeoffs each link introduces — explains how a naive exhaustive search becomes a
~1600 ELO engine.
...
### 10. Texel tuning (piece-square-table optimisation)
